    ...
    */
    public Profile createSingleProfile() {
        ...
        for(int i = 0; i <STRs.length; i++) {
            ...
            STRs[i] = new STR(name, occurrences); 
        }
    ...
    private TreeNode insertion(TreeNode n, String name, Profile profile) {
        if(n==null) {
            return new TreeNode(name, profile, null, null); 
        }

        int valueToCompare = name.compareTo(n.getName());


        if(valueToCompare < 0) {
            n.setLeft(insertion(n.getLeft(), name, profile));
        }

        
        else if(valueToCompare>0) {
            n.setRight(insertion(n.getRight(), name, profile));
        }
    ...
    public void insertPerson(String name, Profile newProfile) {
        ...
        // WRITE YOUR CODE HERE
        treeRoot = insertion(treeRoot, name, newProfile); 
    ...
    private int matchingProfile(TreeNode root, boolean val) {
    ...
    public int getMatchingProfileCount(boolean isOfInterest) {
        ...
        // WRITE YOUR CODE HERE
        return matchingProfile(treeRoot, isOfInterest); 
    ...
    private void traversal(TreeNode root) {
        ...
        if(Math.ceil(STRs.length/2.0)<=count) {
